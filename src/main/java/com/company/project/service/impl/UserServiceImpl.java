...
public class UserServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements UserService {
    ...
    public IPage<SysUser> pageInfo(SysUser vo) {
        ...
        if (!iPage.getRecords().isEmpty()) {
            for (SysUser sysUser : iPage.getRecords()) {
                ...
                List<String> roleNames = userRoleService.getRoleNamesByUserId(sysUser.getId());
                sysUser.setRoleIds(roleNames);
            }
        }
    ...
    public Integer getUserCount(){
        LambdaQueryWrapper<SysUser> queryWrapper = Wrappers.lambdaQuery();
        queryWrapper.eq(SysUser::getStatus, 1);//只查询状态正常的
    ...
    public JSONArray getTeachers(){
        if(!redisService.exists(Constant.TEACHER_DATA_KEY)){
            this.updateRedisTeacherData();
        }
        String teacherDatas = redisService.get(Constant.TEACHER_DATA_KEY);
        return JSONArray.parseArray(teacherDatas);
    }
    ...
    public List<SysUser> getTeacherList(){
        ...
        String teacherDatas = redisService.get(Constant.TEACHER_DATA_KEY);
        return JSONObject.parseArray(teacherDatas, SysUser.class);
    }

    /**
     * 更新redis中的teacher的数据
     */
    public void updateRedisTeacherData(){
        LambdaQueryWrapper<SysUser> queryWrapper = Wrappers.lambdaQuery();
        //queryWrapper.orderByAsc(true, SysUser::get);
        queryWrapper.eq(SysUser::getStatus, 1);//只查询状态正常的
        ...
        list.parallelStream().forEach(bean->{
            List<String> ids = userRoleService.getRoleIdsByUserId(bean.getId());
            if(ids.contains(Constant.TEACHER_ROLE_ID)){
                teacherList.add(bean);
            }
        });
        redisService.set(Constant.TEACHER_DATA_KEY, JSON.toJSONString(teacherList));
    }
